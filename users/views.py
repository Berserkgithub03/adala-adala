...
@login_required
def printnpaiement(request, npaiement_id):
    npaiement = get_object_or_404(npaiements, id=npaiement_id)
    return render(request, 'users/printnpaiement.html', {'npaiement': npaiement})
@login_required
def printjt(request):
...
def subscribe(request):
    if request.method == 'POST':
        ...
        if subscription_plan_name:
            ...
            from_email = 'adaladjango@gmail.com'
...
def update_npaiement(request, npaiement_id):
    ...
    if request.method == 'POST':
        # Update npaiement information based on POST data
        npaiement.fpartie = request.POST.get('nomclient1')
        npaiement.spartie = request.POST.get('nomclient2')
        npaiement.date = request.POST.get('datep')
        npaiement.type = request.POST.get('type')
        npaiement.address = request.POST.get('addressp')
        npaiement.prix = request.POST.get('budjetp')
        npaiement.remarques = request.POST.get('details')
        npaiement.save()
    ...
    return render(request, 'users/upnpaiement.html', {'npaiement': npaiement})
@login_required
